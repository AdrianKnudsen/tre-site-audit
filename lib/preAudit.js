/**
 * Pre-Audit orchestrator — fetches external assets and runs the Python checker.
 *
 * Workflow:
 *   1. assetFetcher.js downloads all external CSS and JS files in parallel
 *   2. HTML, CSS, and JS are written to temp files in /tmp/
 *   3. pre-audit.py is invoked via subprocess with all three files
 *   4. Results JSON is read back and the raw CSS/JS strings are attached
 *      (so claudeAudit.js can send them to Claude for visual checks)
 *   5. Temp files are cleaned up regardless of success or failure
 *
 * By running ~55–60 deterministic checks in Python first, Claude only
 * needs to evaluate the remaining ~40–45 visual/subjective criteria —
 * reducing token usage by ~40–47%.
 *
 * Exports:
