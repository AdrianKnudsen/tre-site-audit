...
async function runLighthouse(url, strategy = 'desktop') {
  ...
  try {
    // Launch Chrome with extra stability flags for resource-heavy or security-strict sites
    const launchOptions = {
      chromeFlags: [
        '--headless',
        '--no-sandbox',
        '--disable-gpu',
        '--disable-dev-shm-usage',   // avoids /dev/shm memory issues on Linux
        '--no-first-run',
        '--disable-extensions',
    ...
    const options = {
      ...
      port: chrome.port,
      maxWaitForLoad: 60000, // 60s for slow or heavy sites (default is 45s)
...
async function runLighthouseWithRetry(url, strategy) {
  try {
    return await runLighthouse(url, strategy);
  } catch (err) {
    console.warn(`[Lighthouse] First attempt failed (${strategy}): ${err.message} — retrying in 3s...`);
...
async function getPageSpeedData(url) {
  ...
  const desktopRaw = await runLighthouseWithRetry(url, 'desktop');
  const mobileRaw = await runLighthouseWithRetry(url, 'mobile');
