...
function generateFailingAuditRows(audits, extraTranslations = {}) {
  ...
  return audits.map(a => {
    ...
    const rawDescFallback = (a.description || '').replace(/\[([^\]]+)\]\([^)]+\)/g, '$1').trim();
    const descNo = escapeHtml((LH_DESCRIPTIONS_NO[a.id] || extra.description || rawDescFallback).substring(0, 150));
    return `<tr>
...
function buildReport(pageSpeedData, claudeData, url, extraTranslations = {}) {
...
export { buildReport, LH_TITLES_NO };
