...
`;
  ...
  function compressHTML(rawHtml) {
    ...
    out = out.replace(/<script([^>]*)>([\s\S]*?)<\/script>/gi, (match, attrs, content) => {
    ...
    out = out.replace(/<style([^>]*)>([\s\S]*?)<\/style>/gi, (match, attrs, content) => {
      if (content.length < 500) return match;
      savings += content.length;
    ...
    out = out.replace(/data:[a-z/\-+]+;base64,[A-Za-z0-9+/=]{100,}/g, (match) => {
      savings += match.length;
      return `data:[${Math.round(match.length / 1024)}KB base64]`;
    });

    if (savings > 0) {
      console.log(`[Claude] HTML compression: removed ${Math.round(savings / 1024)}KB of non-structural content (${Math.round(rawHtml.length / 1024)}KB → ${Math.round(out.length / 1024)}KB)`);
    }
    return out;
  }
  ...
  const systemTokensEst = Math.ceil((SYSTEM_PROMPT.length + AUDIT_CRITERIA.length) / CHARS_PER_TOKEN);
  ...
  const maxHtmlTokens = maxInputTokens - systemTokensEst - promptOverheadTokens - 5000;
  ...
  if (processedHtml.length > maxHtmlChars) {
    ...
    console.log(`[Claude] HTML still over budget after compression — kept head (${Math.round(headChars/1024)}KB) + tail (${Math.round(tailChars/1024)}KB)`);
  }
  ...
  const availableForAssets = Math.max(0, availableTokensForAssets * CHARS_PER_TOKEN);
  ...
  console.log(`[Claude] Raw sizes — HTML: ${Math.round(html.length/1024)}KB (sent: ${htmlSizeKB}KB), CSS: ${Math.round(rawCSS.length/1024)}KB, JS: ${Math.round(rawJS.length/1024)}KB`);
