/**
 * Claude API integration — AI-powered visual and subjective website evaluation.
 *
 * When pre-audit data is available (normal path):
 *   - Builds a focused prompt listing only the NEEDS_AI_REVIEW checks
 *   - Includes pre-audit context: detected frameworks, CSS stats, JS patterns
 *   - Sends full HTML + compressed CSS + compressed JS within the 200K token budget
 *   - After Claude responds, mergeAuditResults() combines pre-audit + Claude findings
 *
 * When pre-audit data is unavailable (fallback):
 *   - Sends the full 101-point criteria list and asks Claude to evaluate everything
 *
 * Token budget management:
 *   - System prompt + criteria list: ~3,000 tokens reserved
 *   - HTML: sent in full (no truncation)
 *   - CSS + JS: compressed (comments stripped, long lines replaced, keyframes collapsed)
 *     then proportionally truncated if they exceed the remaining budget
 *   - Output reserve: 32,000 tokens for the full 101-check JSON
 *
 * In-memory cache: identical audits (same URL + MD5 of HTML) return cached results
 * without a new API call.
 *
 * Exports:
 *   runClaudeAudit(html, url, apiKey, preAuditData?) → claudeData object
 */
...
You will receive the HTML + CSS + JS source code of a webpage. Analyze it thoroughly and evaluate against each criterion in the 101-point audit checklist. The checklist references Nielsen's 10 Usability Heuristics where applicable.

IMPORTANT: You can only evaluate what is observable from the HTML, CSS source code. For visual-only aspects (like actual rendered spacing, color harmony, visual weight), make your best assessment based on CSS classes, inline styles, and structural patterns in the HTML. If something cannot be determined from HTML or CSS alone, mark it as "na" with a note explaining why.
...

{
  ...
  "priorityFixes": [
    { "title_no": "Kort, slagkraftig tittel som navngir problemet direkte", "title_en": "Short punchy title naming the problem directly", "description_no": "1-2 setninger: hva brukeren opplever + hva det koster bedriften konkret (tapte leads, lavere konvertering, svekket tillit, dårligere rangering)", "description_en": "1-2 sentences: what the user experiences + the specific business cost (lost leads, lower conversion, weakened trust, lower ranking)", "severity": "high|medium|low", "domain": "UX|UI|Accessibility|Best Practices" }
  ],
  "customerSummary": {
    "no": "4-5 avsnitt på norsk. Skriv fra «vi»-perspektiv, som et byrå som snakker naturlig til kunden (f.eks. «Vi ser at...», «Analysen vår viser...», «Vi anbefaler...»). ALDRI bruk: tallscorer, prosenter, fargekoder, hex-verdier, tekniske forkortelser (LCP, CLS, TTFB osv.), eller kodesnutter. ALDRI nevn juridiske konsekvenser, lovkrav, WCAG-krav, bøter, søksmål eller liknende — hold fokus utelukkende på brukeropplevelse og forretningsverdi.\n\nSTRUKTUR (følg denne rekkefølgen):\n1. ÅPNING (1 avsnitt): Start positivt — trekk fram det siden gjør bra. Anerkjenn styrker konkret. Dette bygger tillit.\n2. FORBEDRINGSPOTENSIAL (2-3 avsnitt): Vær tydelig og direkte — navngi de viktigste problemene og forklar hva de sannsynligvis koster. Bruk konkrete eksempler på hva brukerne opplever. Bruk assertivt men ærlig språk: «Det er en reell risiko for at...», «Dette kan føre til at...», «Mange brukere vil trolig...». Ikke presenter antatte konsekvenser som garanterte fakta — beskriv dem som sannsynlige utfall basert på funnene. Kunden skal sitte igjen med en klar forståelse av hva som er i spill.\n3. AVSLUTNING (1 avsnitt): Oppsummer det totale potensialet. Gi kunden en følelse av at forbedringene er oppnåelige og vil gi merkbar effekt. Avslutt med en tydelig anbefaling om neste steg — gjør det lett å si ja til en oppfølging.\n\nTONE: Profesjonell, direkte og rådgivende. Vi er på kundens side, men vi er ærlige. Skriv slik at kunden tenker «dette MÅ vi ta tak i» og ønsker å gå videre med dere. Skill avsnittene med \\n\\n.",
    "en": "4-5 paragraphs in English. Write from a 'we' perspective, as an agency speaking naturally to the client (e.g. 'We see that...', 'Our analysis shows...', 'We recommend...'). NEVER use: numeric scores, percentages, color codes, hex values, technical abbreviations (LCP, CLS, TTFB etc.), or code snippets. NEVER mention legal consequences, legal requirements, WCAG compliance obligations, fines, lawsuits or similar — focus exclusively on user experience and business value.\n\nSTRUCTURE (follow this order):\n1. OPENING (1 paragraph): Start positive — highlight what the site does well. Acknowledge strengths specifically. This builds trust.\n2. IMPROVEMENT POTENTIAL (2-3 paragraphs): Be direct — name the key problems and explain what they likely cost. Use concrete examples of what users experience. Use assertive but honest language: 'There is a real risk that...', 'This can lead to...', 'Many users will likely...'. Do not state inferred consequences as guaranteed facts — frame them as probable outcomes based on the issues found.\n3. CLOSING (1 paragraph): Summarize the overall potential. Give the client a sense that improvements are achievable and will have noticeable impact. End with a clear call to action — make it easy for them to say yes to next steps.\n\nTONE: Professional, direct, and advisory. We are on the client's side, but we are honest. Write so the client thinks 'we MUST address this' and wants to move forward with you. Separate paragraphs with \\n\\n."
  },
  "executiveSummary": {
    "no": "2-3 setninger på norsk fra «vi»-perspektiv. Start med det viktigste positive, deretter det største forbedringspotensialet med forretningsverdi. Uten tallscorer, tekniske termer eller juridiske referanser.",
    "en": "2-3 sentences in English from a 'we' perspective. Lead with the most important positive, then the biggest improvement opportunity with business value. Without numeric scores, technical terms or legal references."
  }
}
...
- Priority fixes MUST contain EXACTLY 10 items, sorted by severity (high → medium → low). If you've covered the most critical issues, include medium and lower-severity improvements to reach 10. Each fix MUST have a punchy, action-oriented title (e.g. "Mobilbrukere mister tålmodigheten" not "Optimaliser bilder") and a description that (1) describes the concrete user problem and (2) names a likely business consequence. IMPORTANT: Only state consequences as facts if they are certain from the source code (e.g. "denne lenken er ødelagt"). For likely-but-not-certain outcomes, use hedged language: "dette kan føre til at...", "det er en reell risiko for at...", "mange brukere vil trolig...". Never present inferred consequences as guaranteed facts. NEVER mention legal consequences, fines, or compliance obligations.
- Customer summary and executive summary MUST be written from a natural «vi»-perspective, as an agency talking to their client. NEVER mention legal/juridiske konsekvenser. Must be plain language — no scores, no hex codes, no technical jargon. Be assertive: name the problems clearly and connect them to business loss. The client should feel "this needs fixing" — not just "there's room for improvement".
...
- BE CONCISE: Keep notes under 150 characters, details under 300 characters`;
...
async function runClaudeAudit(html, url, apiKey, preAuditData = null) {
  ...
  const cacheKey =
    url + ":" + crypto.createHash("md5").update(html).digest("hex");
    ...
    for (const [category, findings] of Object.entries(preAuditData.findings)) {
      for (const finding of findings) {
        if (finding.status === "NEEDS_AI_REVIEW") {
          ...
          const hint =
            finding.note_en || finding.note
              ? ` (hint: ${finding.note_en || finding.note})`
              : "";
          needsAI.push(
            `${category}: ${finding.check_en || finding.check}${hint}`,
          );
        } else if (finding.automated) {
          automated.push(
            `${category}: ${finding.check_en || finding.check} [${finding.status}]`,
          );
        }
      }
    }
...
- Depth distribution: ${Object.entries(sm.depthDistribution)
        .map(([d, c]) => `depth ${d}: ${c}`)
        .join(", ")}
...
`;
  ...
  // Send HTML + intelligently compressed CSS/JS to Claude
  const rawCSS = preAuditData?._rawCSS || "";
  const rawJS = preAuditData?._rawJS || "";

  /**
   * HTML compression: strips audit-irrelevant content without losing any structural HTML.
   * Removes inline <script>/<style> block content (already available via pre-audit assets),
   * long SVG path coordinates, and base64 data URIs.
   * ALL tags, attributes, text content, nav, footer, forms, and semantic elements are preserved.
   */
  function compressHTML(rawHtml) {
    let out = rawHtml;
    let savings = 0;

    // Strip inline <script> content — keep JSON-LD (needed for SEO check) and tiny scripts (<500 chars)
    out = out.replace(
      ...
      (match, attrs, content) => {
        ...
        return `<script${attrs}>/* ${Math.round(content.length / 1024)}KB stripped — JS analyzed separately */</script>`;
      },
    );

    // Strip inline <style> content (we have full CSS via pre-audit asset fetcher)
    out = out.replace(
      /<style([^>]*)>([\s\S]*?)<\/style>/gi,
      (match, attrs, content) => {
        if (content.length < 500) return match;
        savings += content.length;
        return `<style${attrs}>/* ${Math.round(content.length / 1024)}KB stripped — CSS analyzed separately */</style>`;
      },
    );

    // Shorten long SVG path data (thousands of coordinates — not audit-relevant)
    out = out.replace(/ d="([^"]{200,})"/g, (_match, path) => {
      savings += path.length;
      return ` d="[${(path.length / 1024).toFixed(1)}KB SVG path]"`;
    });

    // Replace base64 data URIs (embedded images/fonts inline)
    out = out.replace(
      /data:[a-z/\-+]+;base64,[A-Za-z0-9+/=]{100,}/g,
      (match) => {
        savings += match.length;
        return `data:[${Math.round(match.length / 1024)}KB base64]`;
      },
    );

    if (savings > 0) {
      console.log(
        `[Claude] HTML compression: removed ${Math.round(savings / 1024)}KB of non-structural content (${Math.round(rawHtml.length / 1024)}KB → ${Math.round(out.length / 1024)}KB)`,
      );
    }
    return out;
  }

  // Step 1: Compress HTML — strips inline scripts/styles/SVGs, preserves all structural content
  let processedHtml = compressHTML(html);

  // Token budget: use 2 chars/token as conservative estimate.
  // Norwegian text tokenizes at ~2.3 chars/token (ø/æ/å count as 1 token per char vs ~4 for ASCII).
  // Being conservative here prevents 400 errors; extra headroom is wasted rather than overflowing.
  // Claude max = 200K tokens. Reserve: output (32K) + safety margin (5K)
  const CHARS_PER_TOKEN = 2;
  const systemTokensEst = Math.ceil(
    (SYSTEM_PROMPT.length + AUDIT_CRITERIA.length) / CHARS_PER_TOKEN,
  );
  ...
  // Step 2: Last-resort safety net — if HTML is still over budget after compression,
  // keep head+tail so nav AND footer are always visible to Claude
  const maxHtmlTokens =
    maxInputTokens - systemTokensEst - promptOverheadTokens - 5000;
  const maxHtmlChars = maxHtmlTokens * CHARS_PER_TOKEN;
  if (processedHtml.length > maxHtmlChars) {
    const headChars = Math.floor(maxHtmlChars * 0.75);
    const tailChars = maxHtmlChars - headChars;
    processedHtml =
      processedHtml.substring(0, headChars) +
      "\n<!-- ... HTML middle section omitted (last resort after compression) ... -->\n" +
      processedHtml.substring(processedHtml.length - tailChars);
    console.log(
      `[Claude] HTML still over budget after compression — kept head (${Math.round(headChars / 1024)}KB) + tail (${Math.round(tailChars / 1024)}KB)`,
    );
  }

  const htmlSizeKB = Math.round(processedHtml.length / 1024);
  const htmlTokensEst = Math.ceil(processedHtml.length / CHARS_PER_TOKEN);
  const availableTokensForAssets =
    maxInputTokens - systemTokensEst - htmlTokensEst - promptOverheadTokens;
  // Convert back to chars
  const availableForAssets = Math.max(
    0,
    availableTokensForAssets * CHARS_PER_TOKEN,
  );

  console.log(
    `[Claude] Token budget — max input: ${maxInputTokens}, system: ~${systemTokensEst}, HTML: ~${htmlTokensEst}, available for CSS+JS: ~${availableTokensForAssets} tokens (~${Math.round(availableForAssets / 1024)}KB)`,
  );
  console.log(
    `[Claude] Raw sizes — HTML: ${Math.round(html.length / 1024)}KB (sent: ${htmlSizeKB}KB), CSS: ${Math.round(rawCSS.length / 1024)}KB, JS: ${Math.round(rawJS.length / 1024)}KB`,
  );
  ...
  function compressJS(js) {
    ...
    out = out.replace(
      /(\/\/ \[minified line removed\]\n?){2,}/g,
      "// [minified bundle code removed]\n",
    );
    // Truncate long string literals
    out = out.replace(
      /(["'`])([^"'`\n]{150,})\1/g,
      (m, q, c) => `${q}${c.substring(0, 60)}...${q}`,
    );
    // Collapse whitespace
    out = out.replace(/[ \t]+/g, " ");
    out = out.replace(/\n{2,}/g, "\n");
  ...
  console.log(
    `[Claude] After compression — CSS: ${Math.round(processedCSS.length / 1024)}KB (was ${Math.round(cssBefore / 1024)}KB), JS: ${Math.round(processedJS.length / 1024)}KB (was ${Math.round(jsBefore / 1024)}KB)`,
  );
  ...
  if (totalAssetChars > availableForAssets && availableForAssets > 0) {
    ...
    if (processedCSS.length > cssAlloc) {
      processedCSS =
        processedCSS.substring(0, cssAlloc) +
        "\n\n/* ... CSS truncated to fit token limit — pre-audit analyzed the full CSS above */";
      console.log(`[Claude] CSS truncated to ${Math.round(cssAlloc / 1024)}KB`);
    }
    if (processedJS.length > jsAlloc) {
      processedJS =
        processedJS.substring(0, jsAlloc) +
        "\n\n// ... JS truncated to fit token limit — pre-audit analyzed the full JS above";
  ...
  console.log(
    `[Claude] Final payload: HTML ${htmlSizeKB}KB + CSS ${cssSizeKB}KB + JS ${jsSizeKB}KB = ${totalKB}KB`,
  );
...
Evaluate ONLY the ${
        Object.values(preAuditData.findings)
          .flat()
          .filter((f) => f.status === "NEEDS_AI_REVIEW").length
      } checks listed above. Do NOT re-evaluate automated checks.
...
Return JSON with complete audit results.`;
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        "Claude API connection timed out. This may be due to:\n" +
  ...
  try {
    ...
    const result =
      preAuditData && preAuditData.findings
  ...
  for (const [preCategory, findings] of Object.entries(preAuditData.findings)) {
    ...
    for (const preFind of findings) {
      ...
      if (existingIdx >= 0) {
        ...
        if (
          !merged[domain][subdomain][existingIdx].status ||
